"""
Exercise 1: Simple Linear Regression Implementation

In this exercise, you'll implement a simple linear regression model from scratch
using NumPy. You'll create functions to compute the hypothesis, cost, and gradient
descent, and then use them to fit a linear model to some sample data.
"""
import numpy as np
...
def hypothesis(X, theta):
    ...
    Parameters:
    X (numpy.ndarray): Input features with shape (m, n+1) where m is the number of examples
                       and n is the number of features (plus 1 for the bias term).
    theta (numpy.ndarray): Model parameters with shape (n+1,).
    
    Returns:
    numpy.ndarray: Predicted values with shape (m,).
    """
    # TODO: Implement the hypothesis function
    pass
...
if __name__ == "__main__":
    ...
    print(f"Prediction for x=6: {prediction[0]:.4f}" if isinstance(prediction, np.ndarray) else f"Prediction for x=6: {prediction:.4f}")
